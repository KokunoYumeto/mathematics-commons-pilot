...
- Current operational publication branch: local `codex/interlanguage-adapter`, based on protected Phase A commit `A`; no remote adapter branch or pull request exists at this checkpoint
...
- The infrastructure-only interlanguage adapter is audited and locally frozen. It still requires a protected pull request and anonymous public readback.
...
1. Replace the adapter branch's obsolete one-shot Phase A CI assertions with event-bound protected-base validation, prove all Phase A scientific/contract bytes unchanged, and rerun the complete cross-platform-equivalent matrix.
2. Push only `codex/interlanguage-adapter`, open a protected infrastructure pull request, require native Ubuntu and Windows plus aggregate `validate`, merge without rewriting `A`, and anonymously verify the exact adapter and still-submitted 13-record calibration.
...
4. Prepare additional draft intake and packet surfaces that require no fabricated capacity, and keep the public recruitment, candidate-selection, independent-audit, and mirror issues current.
...
This worktree is the separate branch `codex/interlanguage-adapter`, based on exact Phase A. Its audited adapter was frozen locally at implementation commit `9cf2a77101634a5dd3b5b014cb7a5f4bc5317de1`, tree `60879218f28a84159b5b20ea9f663019c7b13b3e`, with receipt closure `c0dcca3f6e6d2ab001eca90b1bc012c58ab5d32b`; no remote adapter branch or pull request exists. The adapter pins human-approved upstream commit `5f41b18467c315aee5f465894dd85a277081c74e`, tree `ac9630785b2f8d0534c9e35da95f957538158a47`, and four exact same-commit files. It records the optional human-board identity and two public audit receipts as explicitly non-input provenance. The application makes no HTTP/API request, renders only inert coordination pointers, exposes claim and handback separately, and cannot create a live Commons record. Floating upstream refs have already advanced beyond `5f41b...` and remain discovery locators only.

The exact 5f41 four-file snapshot, optional board identity, two public receipt identities, all commit-ancestry comparisons, and an independent Draft 2020-12 validation have passed. The corrected focused adapter suite passes 34 tests with one expected local Windows symlink-permission skip; the complete repository passes 135 tests with two expected skips. Seven-schema, 13-record calibration/default/CLI, repository-aware validation, structural/public-artifact, Python compilation, nine-file YAML, three-document source parity, exact-snapshot human/JSON projection, and whitespace/text gates passed before the publication-sequence revision. The revised dynamic protected-base workflow and reader text require a fresh complete rerun before push.

Exact continuation: public issue 10 carries the anonymously verified append-only 5f41 supersession receipt at `https://github.com/KokunoYumeto/mathematics-commons-pilot/issues/10#issuecomment-5231874328`. Finish the event-bound protected-base workflow and revised infrastructure-only reader state, rerun every adapter and repository gate, commit the exact delta, then publish it through its own protected pull request without changing or accepting the calibration. After public readback, archive rows may be inspected as candidate intake; only a steward-created Research Packet can enter the Commons record graph. The eventual Phase B must keep `A` as an unchanged ancestor and still requires a qualified manual review before acceptance.
