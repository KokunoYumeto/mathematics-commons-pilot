...
`jobs.json` is the public runnable-job catalog. `translations.json` is the open-education translation-source catalog. `check.json` is the generated validation receipt. `assets/*.json` binds each release ZIP and every represented source member.
