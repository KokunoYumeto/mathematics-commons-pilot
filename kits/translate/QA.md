...
Return `PASS` only when every declared scope and evidence check passes. Otherwise return `DEFECTS_FOUND` with exact file/unit/page/ID locators. Do not repair the candidate during the cold audit. Production repairs the defects in a new generation and submits that frozen generation to another cold audit.
