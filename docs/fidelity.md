# Transcription fidelity contract

These jobs produce inspectable editions, not summaries. A packet is runnable only when its source and workflow boundary are exact.

## Source authority

- Use the highest-resolution complete authoritative scan named in the packet.
- Preserve its exact physical-page topology, including blank pages and separate supplements.
- Treat OCR, old TeX, inherited translations, mathematical expectation, and model prose as fallible witnesses.
- Record scanner, platform, library-copy, barcode, stamp, binding, and holding notices as provenance, but exclude them from the scholarly body.

## Edition layers

- Transcribe the source language diplomatically at symbol level.
- Never summarize, skip, modernize silently, invent a range, or drop a formula, diagram, table, footnote, or index entry.
- Produce the English or other target-language edition as a separate standalone work.
- Put corrections, conjectural readings, and modern mathematical observations in a separate restrained apparatus.
- Keep different authors, translators, editors, languages, supplements, and historical strata distinct.

## Figures and difficult pages

Reconstruct a figure or table editably only when every visible feature can be reproduced. Otherwise preserve an untouched crop from the authority and add a separately named conservative presentation derivative. The derivative never replaces the authoritative pixels.

## State and interruption safety

Every response returns a cumulative full-state ZIP, checkpoint, and manifest. A later session verifies that exact trio before acting. A partial response is preserved, not summarized away. Decisions, errors, reversals, and superseded generations remain in append-only ledgers.

## Completion gate

A terminal result requires deterministic integration, reproducible builds, complete page and topology accounting, rendered QA, formula/figure/link checks, and a fresh reviewer who audits without patching the candidate. A failed audit freezes that generation and reports the defect; it does not authorize the auditor to repair it invisibly.

Packet validation proves packet integrity and workflow readiness. It is not a claim that the resulting mathematics or translation has already passed these completion gates.
